...
while True:
	...
	retval, oriImg = var.read()
	oriImg = cv.resize(oriImg,(400,400))
	test_image = oriImg

	# test_image = 'Resized_Selfie.jpg'
	...
	try:
		...
		_ = fs3.colorbar(ax3y, cax=cbar_axy)
	except:
		print "Just throwing an error."
	...
	fig = matplotlib.pyplot.gcf()
	cv.imshow("plot",canvas[:,:,[2,1,0]])
	cv.imwrite("plot.jpg",canvas[:,:,[2,1,0]])
	cv.waitKey(25)
